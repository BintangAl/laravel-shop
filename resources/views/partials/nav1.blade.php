<nav class="navbar navbar-expand-lg navbar-dark bg-main-gradient overpass">
    <div class="container">
        ...
        <button class="navbar-toggler shadow-none" type="button" data-bs-toggle="collapse" data-bs-target="#navbar-tool"
            aria-controls="navbar-tool" aria-expanded="false" aria-label="Toggle navigation">
        ...
        <div class="collapse navbar-collapse align-items-center overpass" id="navbar-tool">
            ...
            <ul class="navbar-nav me-auto">
                @auth
                            ...
                            <i class="fa-solid fa-cart-shopping position-relative">
                                @if (count($cart))
                                    <span
                                        ...
                                        style="font-size: 8px">{{ count($cart) }}</span>
                                @endif
                            </i>
                    ...
                    <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="{{ route('purchase', ['status' => 'all']) }}"><i
                                class="fa-solid fa-user"></i> <span
                                class="d-inline-block d-lg-none">{{ auth()->user()->name }}</span></a>
                    </li>
                @else
                    <li class="nav-item">
            ...
            <div class="d-flex w-100 px-0 px-lg-5 me-5 d-block d-lg-none" role="search">
                @include('partials.search', ['mb' => 'mb-0'])
            </div>
        </div>
    </div>
</nav>
