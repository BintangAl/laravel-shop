...
@section('container')
    ...
    <div id="app-field">
        <div class="container overpass mt-3">
                        ...
                        @if (count($tracking))
                            @foreach ($tracking as $key => $value)
                                <div class="d-flex mb-3">
                                    <div class="fs-xsmall text-gray col-2">
                                        {{ date_format(date_create($value->date), 'd M H:i') }}</div>
                                    <i class="fa-solid fa-circle {{ $key == 0 ? 'text-success' : 'text-gray' }} mx-2 mt-1"
            ...
            <button class="btn bg-gray border-0 w-100 mb-5 rounded-0"
                onclick="window.location='{{ route('detail-transaction', [$transaction->invoice]) }}'; load()">Kembali</button>
        </div>
    </div>
